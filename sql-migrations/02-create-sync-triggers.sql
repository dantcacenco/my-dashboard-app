...
-- This creates automatic synchronization between job and proposal statuses
...
BEGIN
  UPDATE jobs 
  SET status = CASE 
    WHEN NEW.status IN ('approved', 'deposit paid') THEN 'scheduled'    WHEN NEW.status IN ('rough-in paid', 'final paid') THEN 'in_progress' 
    WHEN NEW.status = 'completed' THEN 'completed'
