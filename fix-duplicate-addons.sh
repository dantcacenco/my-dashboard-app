...
export default function ProposalEditor({ proposal, customers: initialCustomers, pricingItems: initialPricingItems, userId }: ProposalEditorProps) {
  ...
  const addItem = (item: PricingItem, isAddon: boolean) => {
    ...
    const newItem: ProposalItem = {
      id: `temp-${Date.now()}-${Math.random()}`, // Add random for uniqueness
      name: item.name,
      ...
      is_addon: isAddon,
      is_selected: true // Fixed: was malformed as just "add-on"
    }
  ...
  const updateProposal = async () => {
    ...
    try {
      ...
      // Insert all current items (with deduplication)
      // Use a Map to ensure uniqueness by name + is_addon combination
      ...
      const itemsToInsert = Array.from(uniqueItemsMap.values()).map((item, index) => ({
        ...
        sort_order: index // Add sort order for consistency
      }))
              ...
              {showAddItem && (
                <div className="border-2 border-blue-200 rounded-lg p-4 mb-4 bg-blue-50">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-medium">Add Service or Add-on</h3>
                    <button
                  ...
                  <ServiceSearch 
                    ...
                    onItemSelect={(item) => addItem(item, false)}
                    onAddonSelect={(item) => addItem(item, true)}
                    onAddNewPricing={() => setShowAddNewPricing(true)}
                  />
                </div>
              )}
