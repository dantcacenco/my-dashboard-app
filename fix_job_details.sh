...
# Fix Job Details Page Issues
# 1. Remove Quick Actions box
# 2. Fix Edit Job modal to populate with data
# 3. Add console logging for photo debugging

set -e

echo "============================================"
echo "Fixing Job Details Page Issues"
echo "============================================"

PROJECT_DIR="/Users/dantcacenco/Documents/GitHub/my-dashboard-app"
cd "$PROJECT_DIR"

# First, let's check what the current job details page looks like
echo "Checking current job details page structure..."

# Update the job details page to remove Quick Actions and fix Edit modal
cat > "$PROJECT_DIR/app/(authenticated)/jobs/[id]/page.tsx" << 'EOF'
...
# Create the JobDetailsView component
cat > "$PROJECT_DIR/app/(authenticated)/jobs/[id]/JobDetailsView.tsx" << 'EOF'
'use client'
...
if [ $? -eq 0 ]; then
  ...
  git add -A
  git commit -m "Fix job details page: remove Quick Actions, fix Edit modal, add photo debugging

- Removed Quick Actions box from job details page
- Fixed Edit Job modal to properly populate with job data
- Added extensive console logging for photo debugging
- Added image load testing for each photo
- Fixed modal to allow editing all job fields
- Added proper error handling for failed image loads"
  ...
  echo ""
  echo "============================================"
  echo "SUCCESS! Job details page fixed"
  echo "============================================"
  echo ""
  echo "CHANGES MADE:"
  echo "1. ✅ Removed Quick Actions box"
  echo "2. ✅ Fixed Edit Job modal - now populates with job data"
  echo "3. ✅ Added console logging for photo debugging"
  echo ""
  echo "TO DEBUG PHOTOS:"
  echo "1. Open browser console (F12)"
  echo "2. Navigate to a job details page"
  echo "3. Look for these console messages:"
  echo "   - 'JobDetailsView mounted with:'"
  echo "   - 'Photo X: URL: ...' for each photo"
  echo "   - '✅ Photo loaded' or '❌ Photo failed'"
  echo ""
  echo "The console will show:"
  echo "- Total photo count"
  echo "- Each photo's URL and load status"
  echo "- Any errors loading images"
  echo ""
  echo "Common issues to check:"
  echo "- CORS errors (storage bucket not public)"
  echo "- 404 errors (URL incorrect)"
  echo "- Network errors (firewall/proxy)"
  echo "============================================"
else
