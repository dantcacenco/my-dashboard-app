#!/bin/bash
echo "🔧 Implementing multi-stage payment system..."

# Create backup of CustomerProposalView
cp app/proposal/view/\[token\]/CustomerProposalView.tsx app/proposal/view/\[token\]/CustomerProposalView.tsx.backup

# Update CustomerProposalView with multi-stage payment UI
...
# Check for errors
if [ $? -ne 0 ]; then
    echo "❌ Error writing CustomerProposalView.tsx"
    exit 1
fi

# Update payment success page to redirect back to proposal
echo "🔧 Updating payment success page to redirect back to proposal..."

# Create backup
cp app/proposal/payment-success/page.tsx app/proposal/payment-success/page.tsx.backup
...
# Check for errors
if [ $? -ne 0 ]; then
    echo "❌ Error writing payment success page"
    exit 1
fi

# Update create-payment API to handle payment stages
echo "🔧 Updating create-payment API to include payment stage..."

# Create backup
cp app/api/create-payment/route.ts app/api/create-payment/route.ts.backup

# Update create-payment route
...
# Check for errors
if [ $? -ne 0 ]; then
    echo "❌ Error writing create-payment route"
    exit 1
fi

# Commit and push
git add .
git commit -m "feat: implement multi-stage payment system with 50/30/20 split

- Add payment progress UI to customer proposal view
- Show 3 payment stages: deposit (50%), progress (30%), final (20%)
- Lock stages until previous payment is complete
- Add progress bar showing total payment percentage
- Update payment success page to redirect back to proposal
- Track payment stage in Stripe metadata
- Update proposal status based on payment stage"

git push origin main

echo "✅ Multi-stage payment system implemented successfully!"
echo ""
echo "🎯 What's been added:"
echo "- Three payment boxes appear after proposal approval"
echo "- 50% Deposit (active), 30% Progress (locked), 20% Final (locked)"
echo "- Progress bar shows total paid percentage"
echo "- Each payment updates proposal status and unlocks next stage"
echo "- Payment success redirects back to proposal view"
echo ""
echo "📝 Next steps:"
echo "1. Test the payment flow in a private browser"
echo "2. Ensure Stripe webhook properly updates payment statuses"
echo "3. Consider adding email notifications for each payment stage"