...
echo "🔍 Creating upload diagnostic component..."

cd /Users/dantcacenco/Documents/GitHub/my-dashboard-app

# Create a diagnostic version of PhotoUpload with detailed logging
cat > components/uploads/PhotoUploadDebug.tsx << 'EOF'
'use client'
...
export default function PhotoUploadDebug({ jobId, userId, onUploadComplete }: PhotoUploadProps) {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = files.filter(file => {
      ...
      if (!validTypes.includes(file.type)) {
        const msg = `${file.name} is not a valid image file (type: ${file.type})`
        addLog(msg)
        toast.error(msg)
        return false
      }
      if (file.size > 10 * 1024 * 1024) { // 10MB limit
        const msg = `${file.name} is too large (${(file.size / 1024 / 1024).toFixed(2)}MB, max 10MB)`
        addLog(msg)
        toast.error(msg)
        return false
      }
      addLog(`${file.name} is valid (${(file.size / 1024).toFixed(0)}KB, ${file.type})`)
      return true
    })
    ...
    validFiles.forEach(file => {
      ...
      reader.onloadend = () => {
        ...
        if (newPreviews.length === validFiles.length) {
          ...
          addLog(`Created ${newPreviews.length} preview(s)`)
        }
      }
  ...
  const uploadPhoto = async (file: File) => {
    try {
      ...
      addLog('Uploading to storage bucket: job-photos')
      ...
      // Save to database
      addLog('Saving to database table: job_photos')
      const insertData = {
        job_id: jobId,
        uploaded_by: userId,
        photo_url: publicUrl,
  ...
  const handleUpload = async () => {
    ...
    if (successCount > 0) {
      const msg = `${successCount} photo(s) uploaded successfully`
      addLog(msg)
      toast.success(msg)
      onUploadComplete?.()
    }
    ...
    if (errorCount > 0) {
      const msg = `${errorCount} photo(s) failed to upload`
      addLog(msg)
      toast.error(msg)
    }
  }
...
# Now update JobDetailView to use the debug version temporarily
...
export default function JobDetailView({ job: initialJob, userRole, userId }: JobDetailViewProps) {
  ...
  const getCurrentUser = async () => {
    ...
    if (user) {
      ...
      console.log('[JobDetailView] Current user ID:', user.id)
    }
  }
  ...
  const loadJobPhotos = async () => {
    console.log('[JobDetailView] Loading photos for job:', job.id)
    ...
    if (error) {
      ...
      console.log('[JobDetailView] Loaded photos:', data?.length || 0)
    }
  ...
  const loadJobFiles = async () => {
    console.log('[JobDetailView] Loading files for job:', job.id)
    const { data, error } = await supabase
      .from('job_files')
      .select('*')
      .eq('job_id', job.id)
    ...
    if (error) {
      ...
      console.log('[JobDetailView] Loaded files:', data?.length || 0)
    }
  ...
  const refreshJobMedia = () => {
    console.log('[JobDetailView] Refreshing media...')
    loadJobPhotos()
    loadJobFiles()
  }
...
echo "✅ Created debug version with detailed logging"

# Commit the debug changes
git add -A
git commit -m "Add debug logging for upload functionality

- Created PhotoUploadDebug component with detailed console logging
- Added debug information to track upload process
- Shows job ID and user ID in UI
- Logs each step of storage and database operations"
...
echo ""
echo "📋 INSTRUCTIONS:"
echo "1. Go to the job detail page and open browser console (F12)"
echo "2. Try uploading a photo"
echo "3. Watch the debug log that appears below the upload button"
echo "4. Check browser console for detailed errors"
echo "5. Share the error messages shown"
echo ""
echo "The debug component will show:"
echo "- Each step of the upload process"
echo "- Storage bucket responses"
echo "- Database insert results"
echo "- Any error messages with details"
EOF
