...
export default function JobDetailsView({ job: initialJob, jobPhotos: initialPhotos, jobFiles: initialFiles }: JobDetailsViewProps) {
  ...
  const loadAssignedTechs = async () => {
    const { data } = await supabase
      .from('job_technicians')
      .select('technician_id, profiles!technician_id(id, full_name, email)')
      .eq('job_id', job.id)
    
    if (data && data.length > 0) {
      const techs = data.map(d => d.profiles).filter(Boolean)
      setAssignedTechs(techs)
      console.log('Loaded assigned techs:', techs)
    }
  }
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          total_amount: editedJob.total_amount,
          payment_status: editedJob.payment_status
        })
        .eq('id', job.id)
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                value={editedJob.address || job.customers?.address || ''}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  type="number"
                  step="0.01"
                  value={editedJob.total_amount || ''}
                  onChange={(e) => setEditedJob({...editedJob, total_amount: parseFloat(e.target.value)})}
                  placeholder="0.00"
                />
              </div>
              <div>
                ...
                <select
                  className="w-full p-2 border rounded"
                  value={editedJob.payment_status || 'pending'}
                  onChange={(e) => setEditedJob({...editedJob, payment_status: e.target.value})}
                >
                  <option value="pending">Pending</option>
                  <option value="deposit_paid">Deposit Paid</option>
                  <option value="roughin_paid">Rough-in Paid</option>
