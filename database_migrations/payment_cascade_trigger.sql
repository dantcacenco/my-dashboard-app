-- Improved payment trigger with automatic overflow to next stages
...
BEGIN
  ...
  SET 
    ...
    status = CASE
      WHEN v_total_paid_after >= v_proposal.total THEN 'final_paid'
      WHEN v_final_paid_after > 0 OR v_progress_paid_after >= v_progress_due THEN 'progress_paid'
      WHEN v_deposit_paid_after >= v_deposit_due THEN 'deposit_paid'
      ELSE status
    END
...
DROP TRIGGER IF EXISTS trigger_manual_payment_update_proposal_improved ON manual_payments;
DROP TRIGGER IF EXISTS trigger_manual_payment_update_proposal ON manual_payments;
DROP TRIGGER IF EXISTS trigger_manual_payment_cascade ON manual_payments;

-- Create new trigger
CREATE TRIGGER trigger_manual_payment_cascade
AFTER INSERT ON manual_payments
FOR EACH ROW
EXECUTE FUNCTION update_proposal_on_manual_payment_cascade();