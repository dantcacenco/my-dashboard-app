...
-- Using correct column names: clock_in_time, clock_out_time (not clock_out)
-- ================================================
...
DROP POLICY IF EXISTS "Boss and admin can delete time entries" ON public.job_time_entries;
...
-- Technicians can update their own unclosed time entries (clock_out_time is NULL)
CREATE POLICY "Technicians can update own time entries" ON public.job_time_entries
    FOR UPDATE
    USING (
        (auth.uid() = technician_id AND clock_out_time IS NULL)
        OR auth.uid() IN (
            SELECT id FROM profiles WHERE role IN ('boss', 'admin')
        )
    )
...
DROP POLICY IF EXISTS "Users can manage own photos" ON public.job_photos;
...
-- Using correct column name: recorded_by (not added_by)
-- ================================================
...
-- Drop existing policies if any
DROP POLICY IF EXISTS "Authenticated users can view job materials" ON public.job_materials;
DROP POLICY IF EXISTS "Technicians can add materials to assigned jobs" ON public.job_materials;
DROP POLICY IF EXISTS "Users can update materials" ON public.job_materials;
DROP POLICY IF EXISTS "Boss and admin can delete materials" ON public.job_materials;
...
-- Technicians can update materials they recorded, boss/admin can update any
...
-- Using correct column name: user_id (not created_by)
-- ================================================
...
-- Drop existing policies if any
DROP POLICY IF EXISTS "Authenticated users can view activity logs" ON public.job_activity_log;
DROP POLICY IF EXISTS "Authorized users can create activity logs" ON public.job_activity_log;
